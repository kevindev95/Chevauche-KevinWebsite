...
const Header = ({ goToMain, goToOffres, goToAbout, goToOffreSpeciale, page, scrollToProjects, scrollToHome, activeNav }) => {
  ...
  const handleOffreSpecialeClick = () => {
    if (goToOffreSpeciale) {
      goToOffreSpeciale();
    } else {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#101e2b] shadow-md py-4">
      <div className="px-4 md:px-10 lg:px-16 max-w-6xl mx-auto flex justify-between items-center">
        <h3
          className="text-lg font-bold text-yellow-400 cursor-pointer tracking-wide hover-float"
          onClick={goToMain}
        >
          Chevauché Kevin
        </h3>
        <nav className="hidden md:flex gap-8 items-center">
          <button
            ...
            className={`text-base font-semibold pb-1 transition-colors hover-scale ${
              activeNav === 'home' && page === 'main' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-white/90 hover:text-yellow-400'
            }`}
          >
            Accueil
          </button>
      ...
      {isOpen && (
        <nav className="absolute top-full right-4 mt-2 bg-[#101e2b] shadow-md rounded-md w-48 md:hidden">
          <ul className="list-none p-2 m-0">
            ...
            <li>
              <button onClick={handleOffreSpecialeClick} className="block w-full text-left py-1 px-3 hover:bg-[#2e3235] rounded">Offre Spéciale</button>
            </li>
            <li>
